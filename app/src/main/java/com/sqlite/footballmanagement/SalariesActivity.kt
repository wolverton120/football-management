...
class SalariesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val salariesData = db.getTopEarners()

        val tableLayout = findViewById<TableLayout>(R.id.salariesTable)
        val tvHeader = findViewById<TextView>(R.id.tvSalariesHeader)

        val headerText = "Top Earners"
        startLoopingAnimation(tvHeader, headerText, 12000)

        val headerRow = TableRow(this)
        val headers = arrayOf("Player ID", "Player Name", "Salary")
