...
class GoalkeepersActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_goalkeepers)
        enableEdgeToEdge()
        window.decorView.systemUiVisibility = (View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY
                or View.SYSTEM_UI_FLAG_FULLSCREEN)

        val db = DatabaseHelper(this)
        val cleanSheetsData = db.getTopGoalkeepers()

        val tableLayout = findViewById<TableLayout>(R.id.goalkeepersTable)
        val tvHeader = findViewById<TextView>(R.id.tvGoalkeepersHeader)

        val headerText = "Top Goalkeepers"
        startLoopingAnimation(tvHeader, headerText, 12000)

        val headerRow = TableRow(this)
        val headers = arrayOf("Player ID", "Player Name", "Clean Sheets")
