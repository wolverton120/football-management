...
class AgedPlayersActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_aged_players)
        enableEdgeToEdge()
        window.decorView.systemUiVisibility = (View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY
                or View.SYSTEM_UI_FLAG_FULLSCREEN)

        val db = DatabaseHelper(this)
        val agedPlayersData = db.getMostAged()

        val tableLayout = findViewById<TableLayout>(R.id.agedPlayersTable)
        val tvHeader = findViewById<TextView>(R.id.tvAgedPlayersHeader)

        val headerText = "Most Aged Players"
        startLoopingAnimation(tvHeader, headerText, 12000)

        val headerRow = TableRow(this)
        val headers = arrayOf("Player ID", "Player Name", "Age")
